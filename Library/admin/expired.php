...
<div class="container">
  <?php
    if(isset($_SESSION['login_user']))
    {
      ...
      {
        $var1='<p style="color: green; "><b>RETURNED</b></p>';
        mysqli_query($db, "UPDATE issue_book SET approval='$var1'
        WHERE username='$_POST[username]' and isbn='$_POST[isbn]';");

      }

    }
  ?>
  <h2 style="text-align: center;">Date Expired Books</h2>
  <?php
  $c=0;

    if(isset($_SESSION['login_user']))
    {
      $sql="SELECT student.username, roll, books.isbn, name, author, edition, approval, issue, back FROM student INNER JOIN issue_book
      ON student.username = issue_book.username INNER JOIN books ON issue_book.isbn = books.isbn
      WHERE issue_book.approval != '' and issue_book.approval != 'Yes' ORDER BY back ASC";
